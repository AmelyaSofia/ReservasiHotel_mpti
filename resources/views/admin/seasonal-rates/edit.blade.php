...
@section('page_title', 'Edit Harga Dinamis')
@section('breadcrumb', 'Manajemen Harga > Edit')
        ...
        <form method="POST" action="{{ route('admin.seasonal-rates.update', $seasonalRate) }}">
            ...
            <div class="mb-4">
                ...
                <select name="room_type_id" id="room_type_id" class="mt-1 block w-full border-[#EDE8DC] rounded-md shadow-sm focus:ring-[#B8935A] focus:border-[#B8935A] sm:text-sm" required>
                    ...
                    @foreach($roomTypes as $rt)
                        <option value="{{ $rt->id }}" {{ $seasonalRate->room_type_id == $rt->id ? 'selected' : '' }}>{{ $rt->name }} (Normal: Rp {{ number_format($rt->price_per_night, 0, ',', '.') }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ $seasonalRate->start_date->format('Y-m-d') }}" class="mt-1 block w-full border-[#EDE8DC] rounded-md shadow-sm focus:ring-[#B8935A] focus:border-[#B8935A] sm:text-sm" required>
                    @error('start_date') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="end_date" class="block text-sm font-medium text-[#2A1D14]">Sampai Tanggal</label>
                    <input type="date" name="end_date" id="end_date" value="{{ $seasonalRate->end_date->format('Y-m-d') }}" class="mt-1 block w-full border-[#EDE8DC] rounded-md shadow-sm focus:ring-[#B8935A] focus:border-[#B8935A] sm:text-sm" required>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="price_per_night" id="price_per_night" value="{{ (int) $seasonalRate->price_per_night }}" class="mt-1 block w-full border-[#EDE8DC] rounded-md shadow-sm focus:ring-[#B8935A] focus:border-[#B8935A] sm:text-sm" placeholder="Contoh: 300000" required>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="description" id="description" value="{{ $seasonalRate->description }}" class="mt-1 block w-full border-[#EDE8DC] rounded-md shadow-sm focus:ring-[#B8935A] focus:border-[#B8935A] sm:text-sm" placeholder="Misal: Promo Akhir Tahun / Low Season">
